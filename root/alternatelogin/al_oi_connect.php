...
include($phpbb_root_path . 'alternatelogin/openid/common.' . $phpEx);
...
else if ($response->status == Auth_OpenID_FAILURE) 
{
	...
	trigger_error("OpenID authentication failed: " . $response->message);
} 
else if ($response->status == Auth_OpenID_SUCCESS) 
{
	...
	if (@$sreg['email']) {
		$url_params .= "&openid_email=".escape($sreg['email']);
	}

	if (@$sreg['nickname']) {
		$url_params .= "&openid_nickname=".escape($sreg['nickname']);
	}

	if (@$sreg['fullname']) {
		$url_params .= "&openid_fullname=".escape($sreg['fullname']);
	}
...
else
{
	...
	if($user->data['user_id'] == ANONYMOUS)
	{
            ...
            $url = "{$phpbb_root_path}ucp.$phpEx?mode=register&openid_identifier=" . urlencode($esc_identity);
            if($openid_email != '')
            {
                $url .= $url_params;
            }
